(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/scripts/third/WXHelper.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '1a5393KCIVL4IkLI8+zmaQ5', 'WXHelper', __filename);
// scripts/third/WXHelper.js
...
var WXHelper = {
    ...
    "bonusPoint", "masterPoint", "answerTime"],
    RankHeight: [// 设置子域可视高度
    320],
    shareTicket: null,
    scene: null, //场景值
    ...
    init: function init() {
                ...
                if (!cc.vv.matchData.isMatch() && cc.vv.myself.guideStep == 2) {
                    if (typeof res.query.inviterId != "undefined") {
                        if (self._isReadyGroupRank) {
                            ...
                            if (!self._isShowGroupRank) {
                                if (res.query.inviterInitType == '1' && res.query.inviterInitModule == cc.vv.enum.EnumInitView.EIV_GROUP && res.query.inviterInitType && res.query.inviterInitModule) {
        ...
        wx.onShareAppMessage(function () {
            var query = '';
            if (typeof cc.vv != 'undefined' && typeof cc.vv.myself != 'undefined' && typeof cc.vv.myself.playerId != 'undefined' && typeof cc.vv.myself.playerName != 'undefined') {
    ...
    iswxPlatform: function iswxPlatform() {
        return cc.sys.platform === cc.sys.WECHAT_GAME;
    },
    setClipboardData: function setClipboardData(data, _callback) {
    ...
    showLoading: function showLoading(title, mask) {
    ...
    updateShareMenu: function updateShareMenu(type, _callback) {
        wx.updateShareMenu({
    ...
    getShareInfo: function getShareInfo(type, _callback) {
        var self = this;
        wx.getShareInfo({
            shareTicket: type,
            success: function success(res) {
    ...
    // 主动转发，截屏
    shareAppMessageScn: function shareAppMessageScn(title, shareTicket) {
    ...
    // 主动转发，仅标题
    shareAppMessage: function shareAppMessage(title, imageUrl) {
    ...
    shareAppMessageQuery: function shareAppMessageQuery(title, imageUrl, query, successCallback, failCallback, sceneID) {
    ...
    // 调用微信登录
    _login: function _login(_successCb) {
    ...
    loadLocalSessionID: function loadLocalSessionID() {
    ...
    // 请求用户信息
    _getUserInfo: function _getUserInfo(successCb, failCb) {
        var _this = this;
    ...
    // 登陆时候下载资源
    downloadAllResources: function downloadAllResources(target, callFunc) {
    ...
    // 处理加载资源进度信息
    dealDownloadProgress: function dealDownloadProgress(target, index, completedCount, totalCount) {
    ...
    // 获得进度值
    getProgress: function getProgress(arr) {
        var num = 0;
    ...
    setShareCanvasGameAwardData: function setShareCanvasGameAwardData(command, masterPoint, bonus, totalBonus) {
        wx.postMessage({
            command: command,
    ...
    getCustomShareCanvasToShare: function getCustomShareCanvasToShare(title, query, sceneID) {
    ...
    getScreenCanvasToLocalPath: function getScreenCanvasToLocalPath(successCallback, failCallBack) {
        var self = this;
    ...
    downloadSharedCanvasResources: function downloadSharedCanvasResources() {
        var fs = wx.getFileSystemManager();
        wx.downloadFile({
    ...
    getAnswerRank: function getAnswerRank() {
        wx.postMessage({
    ...
    friendRankPageActiveChange: function friendRankPageActiveChange(index) {
        wx.postMessage({
            command: "pageChange",
            pIndex: index,
            myOpenId: this.openId
    ...
    setRankHeight: function setRankHeight(index) {
        wx.postMessage({
            command: "RankHeight",
            keyData: this.RankHeight[index]
        });
    },
    setConfigFriendRankData: function setConfigFriendRankData(keyArr, bonusData, masterPointData, answerTime) {
        wx.postMessage({
            command: 'setRankData',
            keyData: [{
                key: keyArr[0],
    ...
    setMoveTouchDeltaY: function setMoveTouchDeltaY(deltaY) {
        wx.postMessage({
            command: 'moveTouch',
            y: deltaY
        });
    },
    setMoveTouchOff: function setMoveTouchOff() {
        wx.postMessage({
            command: 'moveTouchOff'
        });
    },
    setSharedCanvasChildrenActive: function setSharedCanvasChildrenActive(pIndex) {
        wx.postMessage({
            command: "shareCanvasChange",
            pIndex: pIndex
        });
    },
    setSharedCanvasSize: function setSharedCanvasSize(width, height) {
        sharedCanvas.width = width;
        sharedCanvas.height = height;
    },
    setSharedCanvasDesignResolution: function setSharedCanvasDesignResolution(width, height) {
        wx.postMessage({
    ...
    getSurpassFriendData: function getSurpassFriendData() {
        wx.postMessage({
    ...
    initLoadGroupRankVieW: function initLoadGroupRankVieW() {
        var self = this;
        wx.postMessage({
            command: "groupRank",
